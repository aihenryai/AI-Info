...
import { Search, ExternalLink, Calendar, Tag, ArrowRight, MessageSquare, Image, BookOpen, Users } from 'lucide-react';
...
const ContentApp = () => {
  ...
  const categories = [
    { name: 'כלי בינה מלאכותית', color: 'bg-blue-500', icon: <MessageSquare className="w-8 h-8" /> },
    { name: 'מדריכים וסרטונים', color: 'bg-green-500', icon: <Image className="w-8 h-8" /> },
    { name: 'קורסים וסדנאות', color: 'bg-purple-500', icon: <BookOpen className="w-8 h-8" /> },
    { name: 'קהילה ועדכונים', color: 'bg-yellow-500', icon: <Users className="w-8 h-8" /> }
  ];
  ...
  const filteredMessages = messages.filter(message => 
    (!selectedCategory || message.categories.includes(selectedCategory)) &&
    (!searchTerm || message.content.includes(searchTerm) || message.title.includes(searchTerm))
  ).sort((a, b) => {
